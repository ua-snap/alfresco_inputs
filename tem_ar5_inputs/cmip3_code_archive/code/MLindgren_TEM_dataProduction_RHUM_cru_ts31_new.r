...
# vap2rhum2 <- function(x, y) {
# 	# using the over-water calculation from Steph McAfee
# 	esa = 6.112*exp(17.62*x/(243.12+x))
# 	hur <- y/esa * 100
# 	hur
# } 

# # here we loop through the conversion and produce a progress bar for the graphic-lovers
# for(i in 1) {
# 	hur.stack <- stack()
# 	pb <- pbCreate(nlayers(tas.stack), progress='text')
# 	for(l in 1:nlayers(tas.stack)){
# 		pbStep(pb, l)
		
# 		# calc rhum
# 		out <- vap2rhum(subset(vap.stack,l,T), subset(vap.stack,l,T))
# 		# make a stack of rhum
# 		hur.stack <- addLayer(hur.stack,out)
...
# create 2 empty character vector objects
hur.l <- character()
tas.l <- character()
# to be used in te final step of downscaling
tas.1km.l <- character()
...
for(y in yearList){
	for(mon in monList){
		...
		tas.1km.l <- append(tas.1km.l,paste("/Data/Base_Data/ALFRESCO_formatted/ALFRESCO_Master_Dataset/ALFRESCO_Model_Input_Datasets/AK_CAN_Inputs/Climate/cru_TS31/historical/tas/tas_mean_C_alf_cru_TS31_",mon,"_",y,".tif",sep=""), after=length(tas.1km.l))
	}
}
######################################################	END FILE LISTING LOOP  	######################################################################################################################
# ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
######################################################	LOOP TO FLIP TO PC Latlong 	##################################################################################################################
# to be used later in the last step of downscaling
tas.1km.stack <- quickStack(tas.1km.l)
...
for(i in 1:12){
	...
	nameholder <- paste(outPath,"hur_cru_ts31_1961_1990/","hur_cru_ts_3_10_pcLL_", month,"_1961_1990.tif",sep="")
...
for(mon in monList){
	...
	l <- append(l, paste(outPath,"hur_cru_ts31_1961_1990/","hur_cru_ts_3_10_pcLL_", mon,"_1961_1990.tif",sep=""), after = length(l))
}
...
for(i in 1:12){
	...
	for(j in monthList){
		...
		nameholder <- paste(outPath,"cru_historical_1901_2009/anomalies_pacific/","hur_cru_ts_3_10_pcLL_anom_",month,"_",yearList[count],".tif",sep="")
...
for(i in yearList){
	for(j in 1:12){
		...
		l <- append(l, paste(outPath,"cru_historical_1901_2009/anomalies_pacific/","hur_cru_ts_3_10_pcLL_anom_",month,"_",i,".tif",sep=""), after = length(l))
	}
}
...
for(i in 1:12){ # this is a loop that creates an index of the monthly files I want to grab from the vap.stack on monthly basis
	...
	for(j in monthList){
		...
		tas.stack.current <- subset(tas.1km.stack, j, drop=T)

		in_xyz <- na.omit(data.frame(in_xy, getValues(hur.stack.current)))

		hur.stack.anom.spline <- interp(x=in_xyz[,1],y=in_xyz[,2],z=in_xyz[,3],xo=seq(min(out_xy[,1]),max(out_xy[,1]),l=ncol(cru.current)), yo=seq(min(out_xy[,2]),max(out_xy[,2]),l=nrow(cru.current)))
		...
		writeRaster(downscaled.month.flip.r, filename=paste(outPath,"downscaled/hur_cru_ts31/10min/","hur_cru_ts31_10min_",month,"_",yearList[count],".tif",sep=""), overwrite=T)
		...
		hur.downscaled <- raster(paste(outPath,"downscaled/hur_cru_ts31/1km/","hur_cru_ts31_1km_",month,"_",yearList[count],".tif",sep=""))
		
		# convert back to vapor pressure
		Es = subset(esa.stack,j,T)
		vap.downscaled = (hur.downscaled*Es)/100

		# write out the set of vapor pressures with the erroneous values
		writeRaster(vap.downscaled, filename=paste(outPath,"downscaled/vap_cru_ts31/1km/","vap_cru_ts31_1km_",month,"_",yearList[count],".tif",sep=""), overwrite=TRUE)


		# now we change the >100 values to 95 as per steph's okaying it
		values(hur.downscaled)[which(getValues(hur.downscaled) > 100)] <- 95

		writeRaster(hur.downscaled, filename=paste(outPath,"downscaled/hur_cru_ts31/1km_set95/","hur_cru_ts31_1km_",month,"_",yearList[count],".tif",sep=""), overwrite=TRUE)
		
		# and convert it again so that we have reference		
		vap.downscaled = (hur.downscaled*Es)/100

		writeRaster(vap.downscaled, filename=paste(outPath,"downscaled/vap_cru_ts31/1km_set95/","vap_cru_ts31_1km_",month,"_",yearList[count],".tif",sep=""), overwrite=TRUE)
		...
		# writeRaster(hur.stack, filename=paste(outPath,"downscaled/vapor/cru_ts31/","vap_ts_31_TEM_1km_",month,"_",yearList[count],".tif",sep=""), overwrite=T)
		# #######################################################################################################################
