...
l <- list.files("/big_storage/malindgren/AIEM/RHUM/tas_cru_10min_1961_1990/grid_pclatlong", pattern=".tif", full.names=T)
crustack <- quickStack(l)

shp <- readShapeSpatial("")
...
for(j in 1:nlayers(crustack.desiredCells.e)){
	...
	tas.stack.anom.spline <- interp(x=in_xyz[,1],y=in_xyz[,2],z=in_xyz[,3],xo=seq(min(out_xy[,1]),max(out_xy[,1]),l=ncol(cur)), yo=seq(min(out_xy[,2]),max(out_xy[,2]),l=nrow(cur)))
	...
	# rasterize it
	nc.interp.r <- raster(nc.interp, xmn=xmin(cur), xmx=xmax(cur), ymn=ymin(cur), ymx=ymax(cur))
	writeRaster(nc.interp.r, filename=paste("/big_storage/malindgren/AIEM/RHUM/tas_cru_10min_1961_1990/cru_10min_splined_extent/",crustack@layernames[j],".tif",sep=""), overwrite=T)

}
nc.interp.r <- raster(hur.stack.anom.spline$z, xmn=xmin(cur), xmx=xmax(cur), ymn=ymin(cur), ymx=ymax(cur))
writeRaster(nc.interp.r, filename=paste("/big_storage/malindgren/AIEM/RHUM/tas_cru_10min_1961_1990/cru_10min_splined_extent/",crustack@layernames[j],"_2.tif",sep=""), overwrite=T)