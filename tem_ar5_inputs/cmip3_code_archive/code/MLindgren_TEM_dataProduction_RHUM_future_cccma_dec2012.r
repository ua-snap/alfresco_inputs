...
hur.l <- listem("/big_storage/malindgren/AIEM/RHUM/anomalies/hur/cccma/","hur_mean_cccma_cgcm3_1_prpAnom_",1850,2100,".tif")
tas.l <- listem("/big_storage/malindgren/AIEM/RHUM/anomalies/tas/cccma/","tas_mean_cccma_cgcm3_1_absAnom_",1850,2100,".tif")
# stack em
...
for(i in 1:12){ # this is a loop that creates an index of the monthly files I want to grab from the vap.stack on monthly basis
	...
	for(j in monthList){
		...
		print("reproj 2 akalb")																												# /big_storage/malindgren/AIEM/RHUM/downscaled/a1b/hur/cccma/10min
		downscaled.month.akalb.hur <- projectRaster(trim(downscaled.month.flip.hur.r), res=20000, crs=newProj, method="ngb", filename=paste(outPath,"downscaled/a1b/hur/cccma/10min/","hur_cccma_cgcm31_ar4_10min_akalb_",month,"_",yearList[count],".tif",sep=""), options="COMPRESS=LZW", overwrite=T)
		...
		vapor.ts31.10min.akalb <- projectRaster(trim(vapor.ts31.10min), res=20000, crs=newProj, method="ngb", filename=paste(outPath,"downscaled/a1b/vap/cccma/10min/","vap_cccma_cgcm31_ar4_10min_akalb_",month,"_",yearList[count],".tif",sep=""), options="COMPRESS=LZW", overwrite=T)
		...
		mask(downscaled.month.akalb.hur.1km, out.mask, filename=paste(outPath,"downscaled/a1b/hur/cccma/1km_mask/","hur_cccma_cgcm31_ar4_1km_akalb_",month,"_",yearList[count],".tif",sep=""), overwrite=T, options="COMPRESS=LZW")
		...
		mask(vapor.ts31.1km.akalb, out.mask, filename=paste(outPath,"downscaled/a1b/vap/cccma/1km_mask/","vap_cccma_cgcm31_ar4_1km_akalb_",month,"_",yearList[count],".tif",sep=""), overwrite=T, options="COMPRESS=LZW")
		d.end <- date()
