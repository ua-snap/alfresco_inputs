...
model= "mpi_echam5" # "mpi_echam5"
model.short = "echam5" #unlist(strsplit(model,"_"))[1]
...
hur.future <- stack(c("/Data/Base_Data/Climate/World/GCM_raw/hur/pcmdi.ipcc4.mpi_echam5.sresa1b.run1.monthly.hur_A1_2001-2050.nc","/Data/Base_Data/Climate/World/GCM_raw/hur/pcmdi.ipcc4.mpi_echam5.sresa1b.run1.monthly.hur_A1_2051-2100.nc")) #/Data/Base_Data/Climate/World/GCM_raw/hur/pcmdi.ipcc4.mpi_echam5.sresa1b.run1.monthly.hur_A1_2001-2050.nc

tas.future <- stack("/Data/Base_Data/Climate/World/GCM_raw/tas/Alaska5/pcmdi.ipcc4.mpi_echam5.sresa1b.run1.monthly.tas_A1.nc") #/Data/Base_Data/Climate/World/GCM_raw/hur/pcmdi.ipcc4.mpi_echam5.sresa1b.run1.monthly.hur_A1_2001-2050.nc

tas.future <- subset(tas.future, 1:1200)

# read in the 20c3m stack
hur.past <- stack("/Data/Base_Data/Climate/World/GCM_raw/hur/pcmdi.ipcc4.mpi_echam5.20c3m.run1.monthly.hur_A1_1960-2009.nc")
tas.past <- stack("/Data/Base_Data/Climate/World/GCM_raw/tas/Alaska5/pcmdi.ipcc4.mpi_echam5.20c3m.run1.monthly.tas_A1.nc")
...
hur.past.select <- subset(hur.past, 13:360)
tas.past.select <- subset(tas.past, 1213:1560)
...
for(i in 1:12){ # this is a loop that creates an index of the monthly files I want to grab from the hur.future on monthly basis
	...
	for(j in monthList){
		...
		# rasterize it
		nc.interp.hur.r <- raster(nc.interp.hur, xmn=xmin(cru.hur.current), xmx=xmax(cru.hur.current), ymn=ymin(cru.hur.current), ymx=ymax(cru.hur.current))
		nc.interp.tas.r <- raster(nc.interp.tas, xmn=xmin(cru.tas.current), xmx=xmax(cru.tas.current), ymn=ymin(cru.tas.current), ymx=ymax(cru.tas.current))
