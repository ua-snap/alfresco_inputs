...
#here we flip the map around so that we have pacific centering
if(any(tas.xyz[,1]<0)) tas.xyz[,1][tas.xyz[,1]<0] <- tas.xyz[,1][tas.xyz[,1]<0]+360
if(any(hur.xyz[,1]<0)) hur.xyz[,1][hur.xyz[,1]<0] <- hur.xyz[,1][hur.xyz[,1]<0]+360
...
l <- list.files("/workspace/UA/malindgren/projects/iem/PHASE2_DATA/WORKING_TEM/hur_cru_10min_1961_1990", pattern="_PCLL.tif", full.names=T)
...
for(i in yearList){
	for(j in 1:12){
		if(nchar(j)<2){ month=paste("0",j,sep="")}else{month=paste(j,sep="")}	
		l <- append(l, paste("/Data/Base_Data/Climate/AK_CAN_1km_from2km/cru_TS31/historical/tas/tas_mean_C_alf_cru_TS31_",month,"_",i,".tif",sep=""), after = length(l))
		l2 <- append(l2, )
	}
}

tas.stack.ds <- quickStack(l)

crustack.1km.desiredCells.e <- quickStack()


in_xy <- coordinates(subset(hur.stack.anom,1,drop=T))
out_xy <- coordinates(subset(crustack.desiredCells.e,1,drop=T))

# this is for the extrapolation needs I have for the tas data
in_xy_cru <- coordinates(crustack.desiredCells.e)
out_xy_cru <- coordinates(crustack.desiredCells.e)
...
for(i in 1:12){ # this is a loop that creates an index of the monthly files I want to grab from the vap.stack on monthly basis
	...
	# this little bit of code is to make sure that the data exist outside the bounds of the land extent
	in_xyz_cru <- data.frame(in_xy_cru,getValues(cru.current))
	in_xyz_cru <- in_xyz_cru[!is.na(in_xyz_cru[,3]),]
	# we are splining to the same resolution so the data is the same, just adding data to the noData areas making for a better coastline for TEM
	cru.current.spline <- interp(x=in_xyz_cru[,1],y=in_xyz_cru[,2],z=in_xyz_cru[,3],xo=seq(min(out_xy_cru[,1]),max(out_xy_cru[,1]),l=ncol(cru.current)), yo=seq(min(out_xy_cru[,2]),max(out_xy_cru[,2]),l=nrow(cru.current))) # , linear=FALSE, extrap=TRUE	
	cru.current.spline.flip <- t(cru.current.spline$z)[ncol(cru.current.spline$z):1,]
	cru.current.spline.r <- raster(cru.current.spline.flip, xmn=xmin(crustack.desiredCells.e), xmx=xmax(crustack.desiredCells.e), ymn=ymin(crustack.desiredCells.e), ymx=ymax(crustack.desiredCells.e))
	writeRaster(cru.current.spline.r, filename=paste("/workspace/UA/malindgren/projects/iem/PHASE2_DATA/WORKING_TEM/hur_cru_10min_1961_1990/cru_10min_splined_extent/","HUR_CRU_10MIN_",i,"_1961_1990_PCLL_spline.tif",sep=""), overwrite=TRUE)
	


	cru.current.spline.r.xyz <- data.frame(coordinates(cru.current.spline.r),getValues(cru.current.spline.r))
	if(any(cru.current.spline.r.xyz[,1]>180)) cru.current.spline.r.xyz[,1][cru.current.spline.r.xyz[,1]>180] <- cru.current.spline.r.xyz[,1][cru.current.spline.r.xyz[,1]>180]-360
	cru.current.spline.r.xyz[,1] <- cru.current.spline.r.xyz[,1]-180

	cru.current.spline.r.xyz2rast <- rasterFromXYZ(cru.current.spline.r.xyz, crs="+proj=longlat +ellps=WGS84 +datum=WGS84 +no_defs", digits=8)
	writeRaster(cru.current.spline.r.xyz2rast, filename=paste("/workspace/UA/malindgren/projects/iem/PHASE2_DATA/WORKING_TEM/hur_cru_10min_1961_1990/cru_10min_splined_extent/","HUR_CRU_10MIN_",i,"_1961_1990_LL_spline.tif",sep=""), overwrite=TRUE)
	...
	for(j in monthList){
		...
		# here we subset the stack of the tas data for the conversion back to vapor pressure later on in the loop
		#  we also project the raster out of the standard
		tas.stack.current <- subset(tas.stack.ds, j, drop=T)
		# tas.stack.current.e <- projectExtent(tas.stack.ds, crs="+proj=longlat +ellps=WGS84 +datum=WGS84 +no_defs")
		# tas.stack.current.r <- projectRaster(subset(tas.stack.ds,j,drop=T), tas.stack.current.e)

		tas.stack.current.flip <- data.frame(coordinates(tas.stack.current))

		# create an XYZ table for the spline
		in_xyz <- data.frame(in_xy, getValues(hur.stack.current))
		# remove the rows where any column has an NA in it.
		in_xyz <- in_xyz[!is.na(in_xyz[,3]),]

		# spline interpolation using AKIMA
		hur.stack.anom.spline <- interp(x=in_xyz[,1],y=in_xyz[,2],z=in_xyz[,3],xo=seq(min(out_xy[,1]),max(out_xy[,1]),l=ncol(cru.current.spline.r)), yo=seq(min(out_xy[,2]),max(out_xy[,2]),l=nrow(cru.current.spline.r))) # , linear=FALSE, extrap=TRUE
		...
		# rasterize it
		nc.interp.r <- raster(nc.interp, xmn=xmin(cru.current.spline.r), xmx=xmax(cru.current.spline.r), ymn=ymin(cru.current.spline.r), ymx=ymax(cru.current.spline.r))
		...
		downscaled.month <- cru.current.spline.r*nc.interp.r
		...
		writeRaster(downscaled.month.flip.r, filename=paste("/workspace/UA/malindgren/projects/iem/PHASE2_DATA/WORKING_TEM/downscaled/historical/hur/latlong/","hur_mean_cru_TS31_10min_",month,"_",yearList[count],".tif",sep=""), overwrite=T)
		...
		gdalCall <- paste("gdalwarp -t_srs '", newProj, "' -tr 1000 1000 -r bilinear -srcnodata -9999 -dstnodata NoData -co 'COMPRESS=LZW' -cutline '/workspace/UA/malindgren/projects/iem/PHASE2_DATA/WORKING_TEM/Extents/AIEM/AIEM_domain.shp' -overwrite ", paste("/workspace/UA/malindgren/projects/iem/PHASE2_DATA/WORKING_TEM/downscaled/historical/hur/latlong/","hur_mean_cru_TS31_10min_",month,"_",yearList[count],".tif",sep="")," " , paste("/workspace/UA/malindgren/projects/iem/PHASE2_DATA/WORKING_TEM/downscaled/historical/hur/","hur_ts_3_10_TEM_",month,"_",yearList[count],".tif",sep=""), sep="")

		# this is how we issue a system call in R.
		system(gdalCall)

		hur.downscaled <- raster(paste("/workspace/UA/malindgren/projects/iem/PHASE2_DATA/WORKING_TEM/downscaled/historical/hur/","hur_ts_3_10_TEM_",month,"_",yearList[count],".tif",sep=""))
		...
		vap.converted = hur.downscaled/ESA*100
		##########################################################################################################

		# write out the new raster file
		writeRaster(vap.converted, filename=paste("/workspace/UA/malindgren/projects/iem/PHASE2_DATA/WORKING_TEM/downscaled/historical/vap/","vap_ts_3_10_TEM_",month,"_",yearList[count],".tif",sep=""))
		#######################################################################################################################
