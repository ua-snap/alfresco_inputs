...
esa.stack = 6.112*exp(((22.46*tas.stack) / (272.62 + tas.stack)))
...
#l <- list.files(paste(outPath,"hur_cru_10min_1961_1990",sep=""), pattern="_PCLL.tif", full.names=T)

# use that list object to stack all the files to a stack
crustack <- stack("/big_storage/malindgren/AIEM/RHUM/hur_cru_10min_1961_1990/hur_cru_10min_1961_1990_pcll_multiband.tif")
...
for(i in 1:12){ # this is a loop that creates an index of the monthly files I want to grab from the vap.stack on monthly basis
	...
	for(j in monthList){
		...
		values(hur.downscaled)[which(getValues(hur.downscaled) > 100)] <- 100
