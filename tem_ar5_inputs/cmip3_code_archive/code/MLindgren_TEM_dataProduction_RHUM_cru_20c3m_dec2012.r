...
for(i in 1:12){ # this is a loop that creates an index of the monthly files I want to grab from the vap.stack on monthly basis
	...
	for(j in monthList){
		...
		tas.mat <- getValues(downscaled.month.akalb.tas) 
		hur.mat <- getValues(downscaled.month.akalb.hur)
		...
		downscaled.month.akalb.hur.1km <- resample(downscaled.month.akalb.hur,out.mask,method="bilinear") #, filename=paste(outPath,"downscaled/hur_cru_ts31/1km/","hur_cru_ts31_10min_akalb_",month,"_",yearList[count],sep=""), overwrite=T, options="COMPRESS=LZW"
		...
		# mask(downscaled.month.akalb.tas.1km, out.mask, filename=paste(outPath,"downscaled/vap_cru_ts31/1km_mask/","vap_cru_ts31_1km_",month,"_",yearList[count],".tif",sep=""), overwrite=T, options="COMPRESS=LZW")
		mask(vapor.ts31.1km.akalb, out.mask, filename=paste(outPath,"downscaled/tas_cru_ts31/1km_mask/","tas_cru_ts31_1km_",month,"_",yearList[count],".tif",sep=""), overwrite=T, options="COMPRESS=LZW")
		print(paste("total_time: ",strsplit(d.begin," ")[[1]][5] - strsplit(d.end," ")[[1]][5],sep=""))
