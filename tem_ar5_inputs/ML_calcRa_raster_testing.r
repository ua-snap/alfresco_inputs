...
outDir<-"/workspace/Shared/Tech_Projects/ALFRESCO_Inputs/project_data/TEM_Data/R_test/"
...
template.map <- raster("/workspace/Shared/Tech_Projects/ALFRESCO_Inputs/project_data/TEM_Data/templates/tas_mean_C_AR5_GFDL-CM3_historical_01_1860.tif")
...
# [ 2015 ] - NEW STUFF FOR TESTING
pts <- rasterToPoints( template.map.lat, spatial=TRUE )
pts@proj4string <- CRS('+init=epsg:3338')
pts_4326 = spTransform( pts, CRS('+init=epsg:4326') )
lats <- coordinates(pts_4326)[,1]
lats.rad <- (lats*pi)/180 # ok this is correct

template.map.new <- template.map
template.map.rad <- setValues( template.map.new, lats.rad )
